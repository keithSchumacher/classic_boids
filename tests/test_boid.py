...
class TestBoid3D:
    ...
    def setup(self):
        ...
        self.input_alphabet_positions = {self.id: self.position, BoidID(2): Vector(np.array([3, 4, 5]))}
        self.input_alphabet_velocities = {
            self.id: self.velocity,
            BoidID(2): Vector(np.array([7, 8, 9])),
        }
        self.input_alphabet = InputAlphabet(
            positions=self.input_alphabet_positions, velocities=self.input_alphabet_velocities
        )

    def test_3d_boid_initialization(self):
        assert self.boid.internal_state == self.internal_state
        assert self.boid.perception_functions == self.perception_functions
        assert self.boid.drive_functions == self.drive_functions
        # Verify that we're working with 3D vectors
        assert len(self.internal_state.position.data) == 3
        assert len(self.internal_state.velocity.data) == 3
    ...
    def test_3d_step(self):
        ...
        # Verify that the results contain 3D vectors
        assert len(expected_internal_state.position.data) == 3
        assert len(expected_internal_state.velocity.data) == 3
