...
def test_compute_drives_3d():
    # Arrange: Create mocks for each drive function with 3D vectors
    separation_mock = Mock(return_value=Vector(np.array([1.0, 1.0, 1.0])))
    alignment_mock = Mock(return_value=Vector(np.array([2.0, 2.0, 2.0])))
    cohesion_mock = Mock(return_value=Vector(np.array([3.0, 3.0, 3.0])))
    ...
    # Assert: Check that the results are as expected
    assert result[DriveName.SEPARATION] == Vector(np.array([1.0, 1.0, 1.0]))
    assert result[DriveName.ALIGNMENT] == Vector(np.array([2.0, 2.0, 2.0]))
    assert result[DriveName.COHESION] == Vector(np.array([3.0, 3.0, 3.0]))
