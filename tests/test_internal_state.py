...
class TestInternalState3D:
    @pytest.fixture(autouse=True)
    def setup(self):
        self.position = Vector(np.array([0.0, 0.0, 0.0]))
        self.velocity = Vector(np.array([1.0, 1.0, 1.0]))
        self.perception_distance = (
            {
                DriveName.SEPARATION: 5.0,
                DriveName.ALIGNMENT: 10.0,
    ...
    def test_3d_internal_state_initialization(self):
        assert self.internal_state.id == BoidID(1)
        assert self.internal_state.position == self.position
        assert self.internal_state.velocity == self.velocity
        assert len(self.internal_state.position) == 3  # Verify it's 3D
        assert len(self.internal_state.velocity) == 3  # Verify it's 3D
        assert self.internal_state.perception_distance == self.perception_distance
        assert self.internal_state.perception_field_of_view == self.perception_field_of_view
        assert self.internal_state.mass == self.mass
        assert self.internal_state.max_achievable_velocity == self.max_velocity
    ...
    def test_3d_internal_state_vector_operations(self):
        new_position = self.internal_state.position + Vector(np.array([2.0, 3.0, 4.0]))
        assert new_position == Vector(np.array([2.0, 3.0, 4.0]))
        new_velocity = self.internal_state.velocity - Vector(np.array([0.5, 0.5, 0.5]))
        assert new_velocity == Vector(np.array([0.5, 0.5, 0.5]))

    def test_3d_internal_state_update(self):
        self.internal_state.position += self.internal_state.position
        assert self.internal_state.position == self.position * 2

    def test_3d_get_output_alphabet(self):
        assert self.internal_state.get_output_alphabet() == (
            self.internal_state.id,
            self.internal_state.position,
            self.internal_state.velocity,
    ...
    def test_3d_internal_state_conforms_to_protocol(self):
        def process_internal_state(state: InternalStateProtocol[Vector, dict[DriveName, float]]):
            assert isinstance(state, InternalStateProtocol)
            assert state.id == self.internal_state.id
            assert len(state.position) == 3  # Verify it's 3D
