...
@pytest.fixture
def input_alphabet():
    positions = {1: Vector(np.array([1, 2])), 2: Vector(np.array([3, 4]))}
    velocities = {1: Vector(np.array([5, 6])), 2: Vector(np.array([7, 8]))}
    return InputAlphabet(positions=positions, velocities=velocities)


def test_get_position(input_alphabet):
    np.testing.assert_array_equal(
        input_alphabet.get_position(1), Vector(np.array([1, 2]))
    )
    np.testing.assert_array_equal(
        input_alphabet.get_position(2), Vector(np.array([3, 4]))
    )


def test_get_velocity(input_alphabet):
    np.testing.assert_array_equal(
        input_alphabet.get_velocity(1), Vector(np.array([5, 6]))
    )
    np.testing.assert_array_equal(
        input_alphabet.get_velocity(2), Vector(np.array([7, 8]))
    )


def test_get_positions(input_alphabet):
    expected_positions = {1: Vector(np.array([1, 2])), 2: Vector(np.array([3, 4]))}
...
def test_get_velocities(input_alphabet):
    expected_velocities = {1: Vector(np.array([5, 6])), 2: Vector(np.array([7, 8]))}
...
def test_get_position_not_found(input_alphabet):
    with pytest.raises(KeyError):
        input_alphabet.get_position(3)


def test_get_velocity_not_found(input_alphabet):
    with pytest.raises(KeyError):
        input_alphabet.get_velocity(3)
